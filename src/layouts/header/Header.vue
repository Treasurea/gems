...
<template>
  <v-app-bar app clipped-left clipped-right color="primary" dark>
    ...
    <!-- <v-btn color="primary" dark depressed @click="toSystem_mange" v-if="true">
   ...
   <template #activator="{ on }">
     <v-btn color="primary" dark depressed v-on="on">
      <v-icon class="header__icon-line-height" left> mdi-home </v-icon>
       ...
       <v-icon v-if="sysMenu" right>mdi-chevron-up</v-icon>
       <v-icon v-else right>mdi-chevron-down</v-icon>
     </v-btn>
   </template>
   <v-data-iterator hide-default-footer :items="[{ text: '', values: sys }]">
       ...
       <v-card v-for="item in props.items" :key="item.text" flat>
         <v-list dense>
           <v-list-item
             ...
             @click="goSysMange(store)"
           >
             <v-list-item-content class="text-body-2 font-weight-medium">
               <span>
                 <v-icon :class="{ header__highlight: store.value === StoreMode }" left small>
                   mdi-cube
                 </v-icon>
                 {{ store.text }}
               </span>
 ...
 <template #activator="{ on }">
   <v-btn color="primary" dark depressed v-on="on">
    <v-icon class="header__icon-line-height" left> mdi-home </v-icon>
     ...
     <v-icon v-if="sysWorkSapceMenu" right>mdi-chevron-up</v-icon>
     <v-icon v-else right>mdi-chevron-down</v-icon>
   </v-btn>
 </template>
 <v-data-iterator hide-default-footer :items="[{ text: '', values: sysWorkSapce }]">
   <template #default="props">
     <v-card v-for="item in props.items" :key="item.text" flat>
       <v-list dense>
         <v-list-item
  ...
  export default {
    ...
    data() {
      return {
        storeMenu: false,
        sysMenu:false,
        sysWorkSapceMenu:false,
        stores: [
          { text: this.$root.$t('header.app_store'), value: 'app' },
          { text: this.$root.$t('header.model_store'), value: 'model' },
        ],
        sys: [
          { text: this.$root.$t('header.platform'), value: 'platform' },
          { text: this.$root.$t('header.observability'), value: 'admin-observe' },
          { text: this.$root.$t('header.g_cluster'), value: 'clusters' },
        ],
        sysWorkSapce:[
          { text: this.$root.$t('header.tenant_workspace'), value: 'tenant_workspace' },
          { text: this.$root.$t('header.observability'), value: 'observability' },
          { text: this.$root.$t('header.id_apply_service'), value: 'id_apply_service' },
        ]
      };
    },
    computed: {
    ...
    methods: {
      ...
      goSysMange(store){
       this.$router.push({name:store.value})
