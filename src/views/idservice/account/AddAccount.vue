      ...
      <v-dialog v-model="dialog" max-width="500px">
        <template #activator="{ on, attrs }">
          <div style="display: flex; align-items: baseline">
            <v-text-field v-model="search" append-icon="mdi-magnify" hide-details label="账户名称/邮箱/手机号" style="width: 100px" />
            <v-select v-model="statusSearch" dense height="20px" :items="status" label="全部状态" outlined style="width: 100px" />
            <v-btn class="mb-2" color="primary" dark v-bind="attrs" v-on="on"> 搜索 </v-btn>
            <v-btn class="mb-2" color="primary" dark v-bind="attrs" v-on="on"> 创建账号 </v-btn>
            <v-btn class="mb-2" color="primary" dark v-bind="attrs" v-on="on"> 导入账号 </v-btn>
          </div>
        </template>
        <v-card>
          <v-card-title>
            ...
            <v-container>
              <v-row style="display:flex;flex-direction:column">
                <v-col cols="12" md="4" sm="6">
                  <v-text-field v-model="newAccountForm.name" label="账号名称" />
                </v-col>
                <v-col cols="12" md="4" sm="6">
                  <v-text-field v-model="newAccountForm.organization" label="所属组织" />
                </v-col>
                <v-col cols="12" md="4" sm="6">
                  <v-text-field v-model="newAccountForm.password" label="账号密码" required :rules="objRules.passwordRules" />
                </v-col>
                <v-col cols="12" md="4" sm="6">
                  <v-text-field v-model="newAccountForm.email" label="邮箱" required :rules="objRules.emailRules" />
                </v-col>
                <v-col cols="12" md="4" sm="6">
                  <v-text-field v-model="newAccountForm.phone_number" label="手机号码" :rules="objRules.phoneRules" />
                </v-col>
                <v-col cols="12" md="4" sm="6">
                  <v-switch v-model="is_enable" :label="`${is_enable==true?'启用':'禁用'}`"></v-switch>
                </v-col>
              </v-row>
            </v-container>
          </v-card-text>
